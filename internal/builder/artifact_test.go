...
// Pruning must survive a platform that has actually scheduled work. Closed
// assignments (and their released leases) reference the snapshot's probe
// targets, so before this was handled the delete failed on a foreign key and
// every build after RETAIN_SNAPSHOTS was exceeded ended in "prune: ...".
func TestPruneWithScheduledWork(t *testing.T) {
	pool := testPool(t)
	ctx := context.Background()
	env := newBuilder(t, pool, writeBview(t))
	if err := env.b.Run(ctx); err != nil {
	...
	var workerID string
	if err := pool.QueryRow(ctx, `insert into registry.worker (id, operator_id, name, state)
		values (gen_random_uuid(), gen_random_uuid(), 'prune-test', 'active')
		returning id`).Scan(&workerID); err != nil {
		t.Fatal(err)
	}
	if _, err := pool.Exec(ctx, `
		with a as (
		  insert into scheduling.assignment
		    (target_id, provider_id, probe_type, interval_seconds, redundancy_group, status)
		  select t.id, t.provider_id, 'icmp', 60, md5('g' || t.id)::uuid, 'closed'
		  from routing.probe_target t returning id)
		insert into scheduling.lease (assignment_id, worker_id, expires_at, released_at)
		select a.id, $1, now(), now() from a`, workerID); err != nil {
		t.Fatal(err)
	}

	// Three more runs with RetainSnapshots=2 pushes the first past the horizon.
	for i := 0; i < 3; i++ {
		if err := env.b.Run(ctx); err != nil {
			t.Fatalf("run %d: %v", i+2, err)
		}
	}
	var orphanAssignments int
	if err := pool.QueryRow(ctx, `select count(*) from scheduling.assignment a
		where not exists (select 1 from routing.probe_target t where t.id = a.target_id)`).
		Scan(&orphanAssignments); err != nil {
		t.Fatal(err)
	}
	if orphanAssignments != 0 {
		t.Fatalf("%d assignments outlived their targets", orphanAssignments)
	}
}
