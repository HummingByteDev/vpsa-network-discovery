...
func ExportSQLite(ctx context.Context, pool *pgxpool.Pool, snapshotID int64, version, minWorkerVersion, path string) (targets int, err error) {
	...
	if _, err := db.ExecContext(ctx, `
		...
		create table prefixes (
		  ...
		  provider_id text not null, geo_country text,
	...
	prefixRows, err := pool.Query(ctx, `
		select p.prefix::text, p.origin_asn, a.provider_id::text, p.geo_country
	...
	insPrefix, err := tx.PrepareContext(ctx,
		"insert into prefixes (prefix, origin_asn, provider_id, geo_country) values (?, ?, ?, ?)")
	if err != nil {
		return 0, err
	}
	for prefixRows.Next() {
		...
		var originASN int64
		var country *string
		if err := prefixRows.Scan(&prefix, &originASN, &providerID, &country); err != nil {
			return 0, err
		}
		if _, err := insPrefix.ExecContext(ctx, prefix, originASN, providerID, country); err != nil {
			return 0, err
		}
	}
	...
	insTarget, err := tx.PrepareContext(ctx,
		"insert into targets (address, provider_id, prefix) values (?, ?, ?)")
	if err != nil {
		return 0, err
	}
	for targetRows.Next() {
		var address, providerID, prefix string
		if err := targetRows.Scan(&address, &providerID, &prefix); err != nil {
			return 0, err
		}
		if _, err := insTarget.ExecContext(ctx, address, providerID, prefix); err != nil {
			return 0, err
		}
		targets++
	}
